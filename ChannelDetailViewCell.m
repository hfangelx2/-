...
@interface ChannelDetailViewCell ();
...
@implementation ChannelDetailViewCell
...
- (void)setAllValues:(ChannelDetailModel *)channelDetailModel
{
    self.titleLabel.text = channelDetailModel.title;
    self.titleLabel.textColor = [UIColor blackColor];
    self.titleLabel.font = [UIFont boldSystemFontOfSize:14];

    self.singerLabel.text = channelDetailModel.artistName;
    self.singerLabel.textColor = [UIColor colorWithHexString:@"33cc99"];
    self.singerLabel.font = [UIFont systemFontOfSize:14];
    
    NSURL *backgroundUrl;
    if (![channelDetailModel.albumImg isEqualToString:@""]) {
        
        backgroundUrl = [NSURL URLWithString:channelDetailModel.albumImg];
        
    } else if (![channelDetailModel.posterPic isEqualToString:@""]) {
        
        backgroundUrl = [NSURL URLWithString:channelDetailModel.posterPic];
        
    } else {
        
        backgroundUrl = [NSURL URLWithString:channelDetailModel.thumbnailPic];
    }
...
//    [self.changeView removeFromSuperview];
    ...
    [self.titleLabel setFrame:CGRectMake(self.contentView.frame.size.width / 5 * 2 + 10, self.contentView.frame.size.height / 3 - 10, 200, 20)];
    
    [self.singerLabel setFrame:CGRectMake(self.contentView.frame.size.width / 5 * 2 + 10, self.contentView.frame.size.height / 3 * 2 - 15, 200, 20)];
