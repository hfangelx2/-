...
- (void)setAllValues:(HotModel *)hotModel
{
    ...
    self.titleLabel.text = hotModel.title;
    self.titleLabel.textColor = [UIColor whiteColor];
    self.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    
    self.singerLabel.text= hotModel.artistName;
    ...
    NSURL *backgroundUrl;
    if (![hotModel.albumImg isEqualToString:@""]) {
        
        backgroundUrl = [NSURL URLWithString:hotModel.albumImg];

    } else if (![hotModel.posterPic isEqualToString:@""]) {
        
        backgroundUrl = [NSURL URLWithString:hotModel.posterPic];
        
    } else {
        
        backgroundUrl = [NSURL URLWithString:hotModel.thumbnailPic];
    }
