...
- (void)setAllValues:(SearchModel *)searchModel
{
    self.titleLabel.text = searchModel.title;
    self.titleLabel.textColor = [UIColor blackColor];
    self.titleLabel.font = [UIFont boldSystemFontOfSize:14];
    
    self.singerLabel.text = searchModel.artistName;
    self.singerLabel.textColor = [UIColor colorWithHexString:@"33ffcc"];
    self.singerLabel.font = [UIFont systemFontOfSize:14];
    
    NSURL *backgroundUrl;
    if (![searchModel.albumImg isEqualToString:@""]) {
        
        backgroundUrl = [NSURL URLWithString:searchModel.albumImg];
        
    } else if (![searchModel.posterPic isEqualToString:@""]) {
        
        backgroundUrl = [NSURL URLWithString:searchModel.posterPic];
        
    } else {
        
        backgroundUrl = [NSURL URLWithString:searchModel.thumbnailPic];
    }
...
- (void)layoutSubviews
{
    ...
    [self.titleLabel setFrame:CGRectMake(self.contentView.frame.size.width / 5 * 2 + 10, self.contentView.frame.size.height / 3, 100, 20)];
    
    [self.singerLabel setFrame:CGRectMake(self.contentView.frame.size.width / 5 * 2 + 10, self.contentView.frame.size.height / 3 * 2, 100, 20)];
