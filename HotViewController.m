...
#import "WebRequest.h"
#import "MJRefresh.h"
#import "CustomView.h"  // 这是一个点穿的视图,可以给scrollView边上添加方法,也可以给tableView上面盖的黑色视图点穿.
#import "VideoPlayController.h"



@interface HotViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate,HotViewCellDelegate>
...
@property(nonatomic,strong)UIView *threePointView;
@property(nonatomic,strong)UIButton *downLoadButton;
@property(nonatomic,strong)UIButton *shareButton;
@property(nonatomic,strong)UIButton *cancelButton;

@property(nonatomic, assign)NSInteger indexPathNumber;
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if(self){
        ...
        self.areaArray = [NSArray arrayWithObjects:@"DAYVIEW_ALL",@"DAYVIEW_ML",@"DAYVIEW_HT", @"DAYVIEW_US",@"DAYVIEW_KR",@"DAYVIEW_JP",nil]; // 放地区参数的数组
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 判断参数是哪个scrollView 因为tableView也继承于scrollView,如果不加判断会导致tableView和ScrollView都执行这个协议方法
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HotViewCell *cell = [tableView dequeueReusableCellWithIdentifier:pool];
    
    if(!cell){
        cell = [[HotViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:pool];
        cell.delegate = self;
    }
    ...
    if(self.modelArray.count > indexPath.row){
        HotModel *hotModel = [_modelArray objectAtIndex:indexPath.row];
        [cell setAllValues:hotModel];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    HotModel *model = [self.modelArray objectAtIndex:indexPath.row];
    ...
    [WebRequest connectWithUrl:Today parmater:_urlDic requestHeader:RequestHeader httpMethod:@"GET" view:self.view block:^(id data) {
        ...
        if(data){
            ...
            for (NSDictionary *dic in [bigDic objectForKey:@"videos"]) {
                ...
                [self.modelArray addObject:hotModel];
            }
...
- (void)createThreePointView
{
    self.threePointView = [[UIView alloc]initWithFrame:CGRectMake(20, self.view.frame.size.height, self.view.frame.size.width - 40, 240)];
    self.threePointView.backgroundColor = [UIColor blackColor];
    self.threePointView.alpha = 0.8;
    ...
    [self.downLoadButton setFrame:CGRectMake(0, 0, 50, 30)];
    [self.threePointView addSubview:self.downLoadButton];
    ...
    [self.shareButton setFrame:CGRectMake(50, 30, 50, 30)];
    [self.threePointView addSubview:self.shareButton];
    
    
    self.cancelButton = [UIButton buttonWithType:UIButtonTypeSystem];
    [self.cancelButton setTitle:@"取消" forState:UIControlStateNormal];
    [self.cancelButton addTarget:self action:@selector(cancelButtonAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.cancelButton setFrame:CGRectMake(100, 60, 50, 30)];
    [self.threePointView addSubview:self.cancelButton];
...
- (void)cancelButtonAction:(id)sender
{
    [UIView animateWithDuration:0.5 animations:^{
        ...
        [self.threePointView setFrame:CGRectMake(20, self.view.frame.size.height, self.view.frame.size.width - 40, 240)];
    }];
}
...
{
    self.indexPathNumber = indexPathNum;
    
     [self.threePointView.superview bringSubviewToFront:self.threePointView];
    
    [UIView animateWithDuration:0.5 animations:^{
        [self.threePointView setFrame:CGRectMake(20, self.view.frame.size.height - 240, self.view.frame.size.width - 40, 240)];
        }];
}
